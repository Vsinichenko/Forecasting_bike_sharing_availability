...
event_colnames_helper = {"demand": "event_count_end", "supply": "event_count_start"}
    ...
    for current_cell in df_helper[city].hex_id.unique():
        for part in part_ls:
            for dep_var in dep_var_ls:
                ...
                collector[model_name]["fact"] = test_sr

                predictions = model_fit.get_forecast(steps=len(test_sr), exog=test_exog_df).predicted_mean
                predictions[predictions < 0] = 0

                collector[model_name]["predictions"] = predictions

                test_exog_df_with_no_events = test_exog_df.copy()
                test_exog_df_with_no_events[event_colnames_helper[dep_var]] = 0

                predictions_with_no_events = model_fit.get_forecast(steps=len(test_sr), exog=test_exog_df_with_no_events).predicted_mean
                predictions_with_no_events[predictions_with_no_events < 0] = 0

                collector[model_name]["predictions_with_no_events"] = predictions_with_no_events
